...
export interface Parser {
	...
	/** @private */
	getConfig(this: Parser): ParserConfig;
	 ...
	 * @param title 链接标题
	 */
	isInterwiki(title: string, config?: ParserConfig): [string, string] | null;
 ...
 * @param dir 子路径
 */
function rootRequire(file: string, dir = "") {
	return require(`${file.includes("/") ? "" : `../${dir}`}${file}`);
}
...
export const Parser: Parser = {
	...
	typeAliases: {
		...
		"ext-attr-dirty": [
			"extension-attr-dirty",
			"ext-attribute-dirty",
			"extension-attribute-dirty",
		],
		...
		"converter-rule-noconvert": [
			"convert-rule-noconvert",
			"conversion-rule-noconvert",
		],
		"converter-rule-variant": [
			"convert-rule-variant",
			"conversion-rule-variant",
		],
	...
	getConfig() {
		if (typeof this.config === "string") {
			this.config = rootRequire(this.config, "config/") as ParserConfig;
	...
	normalizeTitle(
		...
		const token = this.run(() =>
			new Token(title, config).parseOnce(0, include).parseOnce()
		),
			titleObj = new Title(
		...
		this.run(() => {
			...
			try {
				...
				if (e instanceof Error) {
					...
					fs.writeFileSync(
						file,
						stage === this.MAX_STAGE ? wikitext : String(token)
					);
					...
					fs.writeFileSync(
						...
						JSON.stringify(
							{
								stage,
								include: token.getAttribute("include"),
								config: this.config,
							},
							null,
							"\t"
		...
		if (this.debugging) {
			...
			if (restored === wikitext) {
				...
				restored = token!
					...
					.replace(
						/<[^<]+?>|&([lg]t|amp);/gu,
						(_: any, s?: "lt" | "gt" | "amp") => (s ? entities[s] : "")
					);
				process = "渲染HTML";
			}
			if (restored !== wikitext) {
				const {
					promises: { 0: cur, length },
				} = this;
	...
	isInterwiki(title, { interwiki } = Parser.getConfig()) {
		return new RegExp(`^(${interwiki?.join("|")})\\s*:`, "iu").exec(
	...
	reparse(date) {
		const main = fs
			.readdirSync(path.join(__dirname, "errors"))
